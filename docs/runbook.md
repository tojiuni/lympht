...
```
Claude가 작성:  vault kv put secret/foo pass="{{lympht:neunexus/foo#password}}"
                                                ↓ PreToolUse hook
...
# Fields at neunexus/github-webhook:
#   ✓ secret
```
...
- `vault-kv-v2-path`: KV v2 mount(`secret`) 기준 경로. 예: `neunexus/github-webhook`
- `field`: KV secret의 키 이름. `lympht check <path>`로 확인 가능.

### 예시

```bash
# DB 비밀번호
psql "postgresql://app:{{lympht:neunexus/cloudbro/postgres-gopedia#password}}@localhost/db"
...
docker login registry.example.com \
  ...
  -p {{lympht:neunexus/registry#password}}
```
...
| 경로 | 용도 | 필드 확인 |
|------|------|-----------|
| `neunexus/github-webhook` | Tekton EventListener HMAC secret | `lympht check neunexus/github-webhook` |
| `neunexus/registry` | artifacts.toji.homes 레지스트리 인증 | `lympht check neunexus/registry` |
| `neunexus/ghcr` | GitHub Container Registry 인증 | `lympht check neunexus/ghcr` |
| `neunexus/git-ssh` | Git SSH 키 | `lympht check neunexus/git-ssh` |
| `neunexus/cloudbro/postgres-gopedia` | gopedia-bro PostgreSQL | `lympht check neunexus/cloudbro/postgres-gopedia` |
